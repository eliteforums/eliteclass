...
- [~] 5. Create `src/modules/exams/hooks/useProctoring.ts` hook with camera/mic permission request via `getUserMedia`, hardware detection via `enumerateDevices`, stream lifecycle management, interruption handling with re-acquisition, and permission denied blocking state
- [~] 6. Implement stream interruption filtering in `useProctoring` to ensure hardware failures (track ended events) do NOT trigger tab-switch violation recording
- [~] 7. Create `src/modules/exams/components/student/ProctoringOverlay.tsx` with camera preview (160×120px video element, fixed top-right), pulsing red recording indicator with "Proctoring Active" text, and conditional rendering based on feature flags
