    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground text-sm">
          View and manage your standalone assignments.
        </p>
      </div>
...
function StudentAssignmentCard({ assignment, onClick }: { assignment: any; onClick: () => void }) {
  ...
  const isOverdue =
    assignment.due_date && isAfter(new Date(), new Date(assignment.due_date)) && !submission;
...
function AssignmentDetailView({
  ...
  const handleFinalSubmit = async () => {
    ...
    if (allowLink && !allowText && !allowFileUpload) {
      ...
      try {
        new URL(submissionContent.trim());
      } catch {
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-muted-foreground uppercase">
                              {Math.round((res.file_size || 0) / 1024)} KB
                            </p>
                          </div>
                        </div>
              ...
              <CardContent className="space-y-4">
                ...
                {submission.content && (
                  <div className="p-3 rounded-xl bg-muted/30 border text-sm">
                    {/^https?:\/\//i.test(submission.content) ? (
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-primary underline break-all hover:opacity-70"
                        >
                          {submission.content}
                        </a>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {submission.files?.map((file: any) => (
                    <div
                      key={file.id}
                      className="flex items-center gap-3 p-3 border border-border/50 rounded-xl bg-muted/20"
                    >
          ...
          <Card className="border-border/50 shadow-sm overflow-hidden">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1">
                ...
                <p
                  className={`text-sm font-bold ${isOverdue && !submission ? "text-destructive" : "text-foreground"}`}
                >
                  {assignment.due_date
                    ? format(new Date(assignment.due_date), "EEEE, MMM d, yyyy")
                    : "No due date"}
                </p>
              ...
              <CardContent className="space-y-4">
                ...
                {allowFileUpload && (
                  <div className="space-y-3">
                    ...
                    {submissionFiles.map((file, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-2.5 border rounded-lg bg-muted/30 text-xs"
                      >
                        ...
                        <button
                          onClick={() => setSubmissionFiles((f) => f.filter((_, i) => i !== idx))}
                    ...
                    <label
                      className={`flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-xl cursor-pointer transition-all ${uploading ? "opacity-50 cursor-not-allowed" : "hover:border-primary/50 hover:bg-primary/5"}`}
                    >
                      {uploading ? (
                ...
                {isOverdue && (
                  <p className="text-[10px] text-center text-orange-600 font-medium">
                    This assignment is past due. Submission will be marked as late.
                  </p>
                )}
              </CardContent>
            </Card>
